...
/**
 * Slice the pattern across A4 tiles and add each tile (with crop marks +
 * page label) to the PDF as **vector** content.
 *
 * Each tile is generated by cloning the source SVG and rewriting its
 * `viewBox` to expose only the sub-rectangle that should be printed on
 * that page. svg2pdf then walks the SVG tree and emits PDF drawing ops
 * directly — there is no canvas rasterisation, no `toDataURL`, and no
 * PNG. This eliminates the iOS canvas-size limit that produced the
 * "wrong PNG signature" / blank-pattern failures on iPhone and iPad.
 */
export async function addTiledPatternToPdf(
  ...
  // Total pattern in mm at 100% scale (1 SVG unit == 1 mm).
  const patternWmm = patternWidthCm * 10;
  const patternHmm = patternHeightCm * 10;

  // Mount the full pattern SVG once so each tile can clone-and-crop it.
  // `prepareSvgForExport` validates the markup and forces concrete
  // width/height attributes (some mobile browsers drop SVGs without
  // those when serializing through XMLSerializer downstream).
  const source = prepareSvgForExport(svgString);
  ...
  try {
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        // Source rectangle (in pattern mm = SVG units) for this tile.
        ...
        // Build a per-tile cropped clone whose viewBox covers only the
        // sub-rectangle for this page. svg2pdf will draw what's inside
        // that viewBox into the destination rect on the PDF.
        const tile = buildCroppedSvg(source.svg, srcXmm, srcYmm, srcWmm, srcHmm);
        try {
          await renderSvgIntoPdf(tile.svg, pdf, {
            x: margin,
            y: margin,
            width: srcWmm,
            height: srcHmm,
