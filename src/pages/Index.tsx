...
const RESULTS: { src?: string; alt: string; fit: string; fabric: string }[] = [
  { alt: "White regular-fit T-shirt sewn from StitchFlow pattern, photographed flat on wood", fit: "Regular fit", fabric: "100% Cotton, 180 GSM" },
  { alt: "Black relaxed-fit T-shirt sewn from StitchFlow pattern, worn by model", fit: "Relaxed fit", fabric: "Cotton jersey, 200 GSM" },
  { alt: "Heather grey slim T-shirt sewn from StitchFlow pattern, hanging on wall", fit: "Slim fit", fabric: "Cotton/elastane rib" },
];
...
const RealLifeResults = () => (
  <section className="border-t bg-secondary/30">
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold tracking-tight">Real-life results</h2>
        <p className="text-sm text-muted-foreground mt-1">
          T-shirts sewn from patterns generated by StitchFlow. No mock-ups, no renders.
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {RESULTS.map((r, i) => (
          <figure key={i} className="bg-card rounded-md border overflow-hidden">
              ...
              {r.src ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center p-6 text-center">
                  <div className="text-xs text-muted-foreground leading-relaxed">
                    <Shirt className="w-8 h-8 mx-auto mb-3 opacity-40" />
                    Drop a real photo here
                    <div className="mt-1 opacity-70">
                      Upload a JPG of a finished T-shirt and replace this slot.
                    </div>
                  </div>
                </div>
              )}
            ...
            <figcaption className="p-4 text-sm">
              <div className="font-medium text-foreground">{r.fit}</div>
              <div className="text-muted-foreground text-xs mt-0.5">{r.fabric}</div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground mt-2">
                Made using StitchFlow
              </div>
            </figcaption>
          </figure>
        ))}
